...
- API 설계에서도 위와 같은 원칙은 그대로 적용되는데, API 설계자는 클라이언트가 일반적인 흐름 제어용으로 예외를 사용할 일이 없게 해야한다. 만약 특정 상태에서만 호출할 수 있는 메서드라면, 클라이언트가 이를 사용할 때 상태를 만족하지 못했을 때 터지는 예외를 사용해서 흐름 제외를 하게 하면 안되고 미리 상태를 검사할 수 있는 메서드를 제공해야한다. 예를 들면 Iterator 인터페이스의 next와 hasNext 메서드가 있다.
